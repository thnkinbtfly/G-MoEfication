...
class MLPCenter(BlockCenter):
    ...
    def cal_center(self):
        ...
        centers = []
        num_blocks = max(self.labels) + 1 # number of experts
        for i in range(num_blocks):
            centers.append(ffn_weight_norm_[np.array(self.labels) == i, :].mean(0))
        centers = np.array(centers) # num_exerts, 768 (t5-base hidden size)
        ...
        mean_acts = None
        if self.config.mean_template:
            mean_acts = load_hidden_states(self.config.folder, self.config.mean_template.format(self.layer)).cuda()
            if self.config.use_abs:
                margin_fn = torch.abs
            else:
                margin_fn = torch.square
        else:
            margin_fn = lambda x: x
        if self.config.force_use_square:
            margin_fn = torch.square
        ...
        last_epoch = -1
        if isinstance(self.config.act_fn, str):
            act_fn = ACT2FN[self.config.act_fn]
        else:
            act_fn = self.config.act_fn

        for epoch in range(30):
            train_hiddens=train_hiddens[torch.randperm(train_hiddens.size()[0])]
            print(train_hiddens.requires_grad)
            ...
            for i in pbar:
                ...
                with torch.no_grad():
                    acts = act_fn(torch.matmul(input, ffn_weight)).float()
                    if mean_acts is not None:
                        margins = margin_fn(acts - mean_acts)
                        scores = torch.matmul(margins, patterns)
                    else:
                        margins = margin_fn(acts)
                        scores = torch.matmul(margins, patterns)
            ...
            for i in range(hiddens.shape[0] // 10 * 9, hiddens.shape[0], 512):
                with torch.no_grad():
                    ...
                    acts = act_fn(torch.matmul(input, ffn_weight)).float() # 512, 4096
                    ...
                    if mean_acts is not None:
                        ...
                        margins = margin_fn(acts)
                        scores = torch.matmul(margins, patterns) # 512, num_blocks, vary from 0 to 1
            ...
            if cur_acc > save_acc[0]:
                self.del_ckpt(save_epoch[0])
                save_acc = [cur_acc]
                save_epoch = [epoch]
                print("input compl center acc", cur_acc)
                self.acc = save_acc[0]
                sys.stdout.flush()
                self.save(epoch)
        ...
        os.system("cp {0}_{1}_{2} {0}_{1}".format(self.filename, self.type, save_epoch[0]))
        os.system("rm {0}_{1}_{2}".format(self.filename, self.type, save_epoch[0]))
